...
#-----------------------------------        
def Bode(P,omega, Show = True, Decompose = True):
    ...
    :Decompose: Show multiple values in your graph. When set to False : only show P(s).
...
def Bode_list(P_list,omega, names):
    fig, (ax_gain, ax_phase) = plt.subplots(2,1)
    fig.set_figheight(12)
    fig.set_figwidth(22)
    for i, Ps in enumerate(P_list): 
        ax_gain.semilogx(omega,20*np.log10(np.abs(Ps)),label=names[i])
        gain_min = np.min(20*np.log10(np.abs(Ps)/5))
        gain_max = np.max(20*np.log10(np.abs(Ps)*5))
        ax_gain.set_xlim([np.min(omega), np.max(omega)])
        ax_gain.set_ylim([gain_min, gain_max])
        ...
        ax_phase.semilogx(omega, (180/np.pi)*np.unwrap(np.angle(Ps)),label=names[i])
        ax_phase.set_xlim([np.min(omega), np.max(omega)])
        ph_min = np.min((180/np.pi)*np.unwrap(np.angle(Ps))) - 10
        ph_max = np.max((180/np.pi)*np.unwrap(np.angle(Ps))) + 10
        ax_phase.set_ylim([np.max([ph_min, -200]), ph_max])
